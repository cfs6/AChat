...
namespace DB_PROXY
{
    ...
    void createGroup(ImPdu* pPdu, uint32_t conn_uuid)
    {
        ...
        if(msg.ParseFromArray(pPdu->GetBodyData(), pPdu->GetBodyLength()))
        {
            ...
            if(IM::BaseDefine::GroupType_IsValid(nGroupType))
            {
                ...
                LOGE("createGroup.%d create %s, userCnt=%u", nUserId, strGroupName.c_str(), setMember.size());
                
                uint32_t nGroupId = GroupModel::getInstance()->createGroup(nUserId, strGroupName, strGroupAvatar, nGroupType, setMember);
                ...
                LOGE("createGroup.%d create %s, userCnt=%u, result:%d", nUserId, strGroupName.c_str(), setMember.size(), msgResp.result_code());
            ...
            {
                LOGE("invalid group type.userId=%u, groupType=%u, groupName=%s", nUserId, nGroupType, strGroupName.c_str());
            }
        }
        else
        {
    ...
    void getNormalGroupList(ImPdu* pPdu, uint32_t conn_uuid)
    {
        ...
        if(msg.ParseFromArray(pPdu->GetBodyData(), pPdu->GetBodyLength()))
        {
            ...
            GroupModel::getInstance()->getUserGroup(nUserId, lsGroup, IM::BaseDefine::GROUP_TYPE_NORMAL);
            msgResp.set_user_id(nUserId);
            ...
            LOGE("getNormalGroupList. userId=%u, count=%d", nUserId, msgResp.group_version_list_size());
    ...
    void getGroupInfo(ImPdu* pPdu, uint32_t conn_uuid)
    {
        ...
        if(msg.ParseFromArray(pPdu->GetBodyData(), pPdu->GetBodyLength()))
        {
            ...
            for(uint32_t i=0; i<nGroupCnt; ++i)
            {
                ...
                if(GroupModel::getInstance()->isValidateGroupId(groupInfo.group_id()))
                {
                    mapGroupId[groupInfo.group_id()] = groupInfo;
                }
            }
            list<IM::BaseDefine::GroupInfo> lsGroupInfo;
            GroupModel::getInstance()->getGroupInfo(mapGroupId, lsGroupInfo);
    ...
    //            *pGroupInfo = *it;
            ...
            LOGE("userId=%u, requestCount=%u", nUserId, nGroupCnt);
    ...
    void modifyMember(ImPdu* pPdu, uint32_t conn_uuid)
    {
        ...
        if(msg.ParseFromArray(pPdu->GetBodyData(), pPdu->GetBodyLength()))
        {
            ...
            if (IM::BaseDefine::GroupModifyType_IsValid(nType) &&
                GroupModel::getInstance()->isValidateGroupId(nGroupId)) {
                ...
                bool bRet = GroupModel::getInstance()->modifyGroupMember(nUserId, nGroupId, nType, setUserId, lsCurUserId);
                msgResp.set_user_id(nUserId);
                msgResp.set_group_id(nGroupId);
                ...
                LOGE("userId=%u, groupId=%u, result=%u, changeCount:%u, currentCount=%u",nUserId, nGroupId,  bRet?0:1, msgResp.chg_user_id_list_size(), msgResp.cur_user_id_list_size());
                msgResp.set_attach_data(msg.attach_data());
                pPduRes->SetPBMsg(&msgResp);
                pPduRes->SetSeqNum(pPdu->GetSeqNum());
                pPduRes->SetServiceId(IM::BaseDefine::SID_GROUP);
            ...
            {
                LOGE("invalid groupModifyType or groupId. userId=%u, groupId=%u, groupModifyType=%u", nUserId, nGroupId, nType);
            }
    ...
    void setGroupPush(ImPdu* pPdu, uint32_t conn_uuid)
    {
        ...
        if(msg.ParseFromArray(pPdu->GetBodyData(), pPdu->GetBodyLength()))
        {
            ...
            if(GroupModel::getInstance()->isValidateGroupId(nGroupId))
            {
                
                ImPdu* pPduRes = new ImPdu;
                bool bRet = GroupModel::getInstance()->setPush(nUserId, nGroupId, IM_GROUP_SETTING_PUSH, nStatus);
                
                msgResp.set_user_id(nUserId);
                msgResp.set_group_id(nGroupId);
                msgResp.set_result_code(bRet?0:1);
            
                LOGE("userId=%u, groupId=%u, result=%u", nUserId, nGroupId, msgResp.result_code());
            ...
            {
                LOGE("Invalid group.userId=%u, groupId=%u", nUserId, nGroupId);
            }
        }
        else
        {
    ...
    void getGroupPush(ImPdu* pPdu, uint32_t conn_uuid)
    {
        ...
        if(msg.ParseFromArray(pPdu->GetBodyData(), pPdu->GetBodyLength()))
        {
            ...
            if(GroupModel::getInstance()->isValidateGroupId(nGroupId))
            {
                ...
                GroupModel::getInstance()->getPush(nGroupId, lsUser, lsPush);
        ...
        //            *pStatus = *it;
                ...
                LOGE("groupId=%u, count=%u", nGroupId, nUserCnt);
            ...
            {
                LOGE("Invalid groupId. nGroupId=%u", nGroupId);
            }
        }
        else
        {
