...
GroupChat* GroupChat::s_group_chat_instance = NULL;

GroupChat* GroupChat::GetInstance()
{
	if (!s_group_chat_instance) {
		s_group_chat_instance = new GroupChat();
	}

	return s_group_chat_instance;
}

void GroupChat::HandleClientGroupNormalRequest(ImPdu* pPdu, CMsgConn* pFromConn)
{
...
void GroupChat::HandleGroupNormalResponse(ImPdu* pPdu)
{
    ...
    pPdu->SetPBMsg(&msg);
    CMsgConn* pConn = ImUserManager::GetInstance()->GetMsgConnByHandle(user_id, attach_data.GetHandle());
    if (pConn)
    {
        pConn->SendPdu(pPdu);
    }
...
void GroupChat::HandleClientGroupInfoRequest(ImPdu *pPdu, CMsgConn* pFromConn)
{
...
void GroupChat::HandleGroupInfoResponse(ImPdu* pPdu)
{
    ...
    if (pduAttachData.GetPduLength() > 0 && group_cnt > 0)
    {
        ...
        for (uint32_t i = 0; i < group_info.group_member_list_size(); i++)
        {
            ...
            ImUser* pToImUser = ImUserManager::GetInstance()->GetImUserById(member_user_id);
            if (pToImUser)
            {
                ...
                if( member_user_id == user_id )
                {
                    ...
                    if(reqHandle != 0)
                        pFromConn = ImUserManager::GetInstance()->GetMsgConnByHandle(user_id, reqHandle);
                }
    ...
    {
        //正常获取群信息的返回
        CMsgConn* pConn = ImUserManager::GetInstance()->GetMsgConnByHandle(user_id, pduAttachData.GetHandle());
        if (pConn)
        {
            msg.clear_attach_data();
            pPdu->SetPBMsg(&msg);
...
void GroupChat::HandleGroupMessage(ImPdu* pPdu)
{
    ...
    CMsgConn* pFromConn = ImUserManager::GetInstance()->GetMsgConnByHandle(from_user_id,
...
void GroupChat::HandleGroupMessageBroadcast(ImPdu *pPdu)
{
    IM::Message::IMMsgData msg;
    CHECK_PB_PARSE_MSG(msg.ParseFromArray(pPdu->GetBodyData(), pPdu->GetBodyLength()));
...
void GroupChat::HandleClientGroupCreateRequest(ImPdu* pPdu, CMsgConn* pFromConn)
{
...
void GroupChat::HandleGroupCreateResponse(ImPdu* pPdu)
{
    ...
    CMsgConn* pFromConn = ImUserManager::GetInstance()->GetMsgConnByHandle(user_id,
                                                                    attach_data.GetHandle());
    if (pFromConn)
    {
        msg.clear_attach_data();
...
void GroupChat::HandleClientGroupChangeMemberRequest(ImPdu* pPdu, CMsgConn* pFromConn)
{
...
void GroupChat::HandleGroupChangeMemberResponse(ImPdu* pPdu)
{
...
void GroupChat::HandleGroupChangeMemberBroadcast(ImPdu* pPdu)
{
...
void GroupChat::HandleClientGroupShieldGroupRequest(ImPdu *pPdu, CMsgConn *pFromConn)
{
...
void GroupChat::HandleGroupShieldGroupResponse(ImPdu *pPdu)
{
    ...
    CDbAttachData attach_data((uchar_t*)msg.attach_data().c_str(), msg.attach_data().length());
    CMsgConn* pMsgConn = ImUserManager::GetInstance()->GetMsgConnByHandle(user_id,
                                        attach_data.GetHandle());
    if (pMsgConn) {
        msg.clear_attach_data();
        pPdu->SetPBMsg(&msg);
...
void GroupChat::HandleGroupGetShieldByGroupResponse(ImPdu *pPdu)
{
...
void GroupChat::_SendPduToUser(ImPdu* pPdu, uint32_t user_id, CMsgConn* pReqConn)
{
    if(!pPdu) {
        return;
    }
    ImUser* pToUser = ImUserManager::GetInstance()->GetImUserById(user_id);
    if(pToUser) {
