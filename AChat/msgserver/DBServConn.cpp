...
static void db_server_conn_timer_callback(void* callback_data, uint8_t msg, uint32_t handle, void* pParam)
{
	...
	DBServConn* pConn = NULL;
	uint64_t cur_time = get_tick_count();

	for (connMap::iterator it = g_db_server_conn_map.begin(); it != g_db_server_conn_map.end(); ) {
		it_old = it;
		it++;

		pConn = (DBServConn*)it_old->second;
		if (pConn->IsOpen()) {
	...
	serv_check_reconnect<DBServConn>(g_db_server_list, g_db_server_count);
}
...
static DBServConn* get_db_server_conn_in_range(uint32_t start_pos, uint32_t stop_pos)
{
	uint32_t i = 0;
	...
	for (i = start_pos; i < stop_pos; i++) {
		pDbConn = (DBServConn*)g_db_server_list[i].serv_conn;
...
DBServConn* get_db_serv_conn_for_login()
{
	...
	DBServConn* pDBConn = get_db_server_conn_in_range(0, g_db_server_login_count);
	if (!pDBConn) {
...
DBServConn* get_db_serv_conn()
{
	...
	DBServConn* pDBConn = get_db_server_conn_in_range(g_db_server_login_count, g_db_server_count);
	if (!pDBConn) {
...
DBServConn::DBServConn(const std::shared_ptr<TcpConnection>& conn, int connid):
					   m_bOpen(false), dbconnId(connid)
{
//	m_bOpen = false;
}
...
void DBServConn::onConnect()
{
	shared_ptr<TcpConnection> conn = getConnectionPtr();
	if(conn)
	{
		dbServConnTimerId = conn->getLoop()->runEvery(1000,std::bind(&db_server_conn_timer_callback,
				this, conn));
	}
}

void DBServConn::Connect(const char* server_ip, uint16_t server_port, uint32_t serv_idx)
{
...
void DBServConn::Close()
{
	...
	serv_reset<DBServConn>(g_db_server_list, g_db_server_count, m_serv_idx);
...
void DBServConn::OnTimer(uint64_t curr_tick)
{
...
void DBServConn::HandlePdu(ImPdu* pPdu)
{
...
void DBServConn::_HandleValidateResponse(ImPdu* pPdu)
{
    ...
    if (result == 0)
    {
        ...
        RouteServConn* pRouteConn = get_route_serv_conn();
        if (pRouteConn) {
...
void DBServConn::_HandleRecentSessionResponse(ImPdu *pPdu)
{
...
void DBServConn::_HandleAllUserResponse(ImPdu *pPdu)
{
...
void DBServConn::_HandleGetMsgListResponse(ImPdu *pPdu)
{
...
void DBServConn::_HandleGetMsgByIdResponse(ImPdu *pPdu)
{
...
void DBServConn::_HandleMsgData(ImPdu *pPdu)
{
...
void DBServConn::_HandleGetLatestMsgIDRsp(ImPdu *pPdu)
{
...
void DBServConn::_HandleUnreadMsgCountResponse(ImPdu* pPdu)
{
...
void DBServConn::_HandleUsersInfoResponse(ImPdu* pPdu)
{
...
void DBServConn::_HandleStopReceivePacket(ImPdu* pPdu)
{
...
void DBServConn::_HandleRemoveSessionResponse(ImPdu* pPdu)
{
...
void DBServConn::_HandleChangeAvatarResponse(ImPdu* pPdu)
{
...
void DBServConn::_HandleDepartmentResponse(ImPdu *pPdu)
{
...
void DBServConn::_HandleSetDeviceTokenResponse(ImPdu *pPdu)
{
...
void DBServConn::_HandleGetDeviceTokenResponse(ImPdu *pPdu)
{
    ...
    for (uint32_t i = 0; i < user_token_cnt; i++)
    {
        ...
        else
        {
            ...
            RouteServConn* route_conn = get_route_serv_conn();
            if (route_conn)
            {
...
void DBServConn::_HandleChangeSignInfoResponse(ImPdu* pPdu) {
...
void DBServConn::_HandlePushShieldResponse(ImPdu* pPdu) {
...
void DBServConn::_HandleQueryPushShieldResponse(ImPdu* pPdu) {
